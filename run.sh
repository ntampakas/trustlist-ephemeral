...
aws deploy create-deployment-group \
  --application-name drill-1 \
  --deployment-config-name CodeDeployDefault.OneAtATime \
  --deployment-group-name drill-$ACTOR \
  --ec2-tag-filters Key=Name,Value=drill-$ACTOR-*,Type=KEY_AND_VALUE \
  --service-role-arn arn:aws:iam::490752553772:role/Drill-deploy || exit 0
